...
with tab3:
    st.header("🔍 Buscador de Evidencias")
    if os.path.exists(ARCHIVO_DATOS):
        ...
        with f_col3:
            busq_texto = st.text_input("Buscar por nombre de archivo...")
                ...
                for arc in lista_files:
                    if busq_texto.lower() in arc.lower() or busq_texto == "":
                        with st.expander(f"📄 {arc} | {fila['Grupo']}"):
                            st.write(f"**Investigador:** {fila['Investigador']}")
                            path = os.path.join(CARPETA_ANEXOS, arc)
                            if os.path.exists(path):
                                with open(path, "rb") as file_bytes:
                                    st.download_button("Descargar", file_bytes, file_name=arc, key=f"{arc}_{idx}")
